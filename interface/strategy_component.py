...
from strategies import TechnicalStrategy, SureForexHedgingStrategy
from utils import *
...
class StrategyEditor(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceClient, bitmex: BitmexClient, *args, **kwargs):
        ...
        self._base_params = [
            {"code_name": "strategy_type", "widget": tk.OptionMenu, "data_type": str,
             "values": ["Technical", "SureForexHedging"], "width": 10, "header": "Strategy"},
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            elif strat_selected == "SureForexHedging":
                new_strategy = SureForexHedgingStrategy(self._exchanges[exchange], contract, exchange, timeframe, balance_pct,
                                                take_profit, stop_loss, self.additional_parameters[b_index])
            else:
                return
